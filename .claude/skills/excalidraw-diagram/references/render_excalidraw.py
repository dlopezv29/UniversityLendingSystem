...
def render(
    ...
    with sync_playwright() as p:
        ...
        # Workaround: esm.sh serves a broken 404 for the transitive
        # @braintree/sanitize-url .../dist/constants.mjs module referenced by the
        # excalidraw bundle. Fulfill it with a valid stub so module load succeeds.
        _sanitize_stub = (
            ...
            "function sanitizeUrl(u){const s=(u||'').replace(htmlCtrlEntityRegex,'').replace(ctrlCharactersRegex,'').trim();"
            "if(!s)return BLANK_URL;if(relativeFirstCharacters.indexOf(s[0])>-1)return s;"
            "const m=s.match(urlSchemeRegex);if(!m)return s;return invalidProtocolRegex.test(m[0])?BLANK_URL:s;}"
            "export{invalidProtocolRegex,htmlEntitiesRegex,htmlCtrlEntityRegex,ctrlCharactersRegex,"
            "urlSchemeRegex,relativeFirstCharacters,BLANK_URL,sanitizeUrl};export default sanitizeUrl;"
        )

        def _fulfill_sanitize(route):
            route.fulfill(status=200, content_type="application/javascript", body=_sanitize_stub)

        page.route("**/sanitize-url@*/**/constants.mjs", _fulfill_sanitize)
